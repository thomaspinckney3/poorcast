...
def test_estate_tax_applies_above_the_exemption_only():
    from poorcast.simulate import Account

    idx = pd.period_range("1960-01", periods=480, freq="M")
    panel = pd.DataFrame({
        "us_equities": np.full(480, 0.006), "inflation": np.zeros(480),
        "income_us_equities": np.zeros(480),
    }, index=idx)
    base = dict(allocation={"us_equities": 1.0}, initial=1_000_000.0, years=10,
                n_sims=50, seed=2)
    # no exemption configured -> not modeled at all
    assert simulate(panel, SimConfig(**base)).estate_tax_real is None
    # an exemption far above any outcome -> zero tax, but still reported
    lo = simulate(panel, SimConfig(**base, estate_exemption=1e12))
    assert lo.estate_tax_real is not None and (lo.estate_tax_real == 0).all()
    # a low exemption -> 40% of the excess
    hi = simulate(panel, SimConfig(**base, estate_exemption=1_000_000.0))
    estate = hi.real_balance[:, -1]
    assert hi.estate_tax_real == pytest.approx(
        0.40 * np.maximum(estate - 1_000_000.0, 0.0)
    )


def test_estate_tax_counts_the_residence():
    idx = pd.period_range("1960-01", periods=480, freq="M")
    panel = pd.DataFrame({
        "us_equities": np.zeros(480), "inflation": np.zeros(480),
        "income_us_equities": np.zeros(480),
    }, index=idx)
    base = dict(allocation={"us_equities": 1.0}, initial=1_000_000.0, years=10,
                n_sims=20, seed=1, estate_exemption=1_000_000.0,
                house_real_growth=0.0)
    without = simulate(panel, SimConfig(**base))
    withh = simulate(panel, SimConfig(**base, house_value=500_000.0))
    # the house adds $500k of taxable estate -> $200k more tax at 40%
    assert np.median(withh.estate_tax_real) - np.median(without.estate_tax_real) \
        == pytest.approx(0.40 * 500_000.0, rel=1e-6)
